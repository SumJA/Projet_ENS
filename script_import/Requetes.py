...
class ImportData:
    ...
    def __init__(self, path):
        ...
        """
        self.get_data = get_data.GetData(path)
    ...
    def import_families(self):
        for fg in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
    ...
    def import_genes(self):  # marche pas encore car faut récupérer nom des gènes
        for family in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
            for key in self.get_data.extract_gene_and_alignment_for_family(family).keys():
    ...
    def import_alignments(
        ...
        for family in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
            for value in self.get_data.extract_gene_and_alignment_for_family(family).values():
    ...
    def import_tree(
            self):  # marche pas car dans base de données c'est "VARCHAR 45" sauf que les alignments sont plus longs et que je n'arrive pas à y changer dans la bdd
        for family in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
            for tree in self.get_data.extract_tree(family):
    ...
    def import_expressions(self):  # marche pas encore car faut remplir les clés étrangères
        for family in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
            directory = os.listdir(self.get_data._expression_level_path)
            for element in directory:
                if not os.path.isdir(element):
                    if element.endswith("quant.tsv"):
                        exps = self.get_data.extract_expression_levels(element, family)
                        for exp in exps:
                            length = exp[0]
                            eff_length = exp[1]
                            est_counts = exp[2]
    ...
    def import_species(self):  # marche pas car duplication --> à gérer
        for family in list(set(self.get_data._extract_families_names(self.get_data._alignments_path))):
            for specie in self.get_data.extract_species_for_gene(family).values():
        ...
        for element in directory:
            if not os.path.isdir(element):
                if element.endswith("metadata.tsv"):
                    exps_info = self.get_data.extract_expression_info(
                        self.get_data._expression_level_path + "/" + element)
