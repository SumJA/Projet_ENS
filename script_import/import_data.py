...
class ImportData:
    ...
    def import_species(self, species_filename: str):
        ...
        for i in species:
            ...
            select_species = """SELECT COUNT(*) FROM species WHERE species_name = '%s' and Species_Taxid = '%s'and Assembly_name = '%s' and Assembly_source ='%s' AND Species_Classification = '%s';""" % (
                sp_name, sp_taxid, asbly_name, asbly_source, sp_classification)
            add_species = """INSERT INTO species(species_name, Species_Taxid, Assembly_name, Assembly_source, Species_Classification) VALUES ('%s', '%s', '%s', '%s', '%s');""" % (
                sp_name, sp_taxid, asbly_name, asbly_source, sp_classification)
            try:
    ...
    def import_expression_method(self):
        ...
        for info in conditions:
            ...
            select_mthd = """SELECT COUNT(*) FROM expression_method WHERE expression_method_name = '%s' AND quantification_tool = '%s';""" % (
                method_expr, tool)
            add_method = """INSERT INTO expression_method(expression_method_name, quantification_tool) VALUES ('%s','%s')""" % (
                method_expr, tool)
            try:
    ...
    def import_condition(self):
        ...
        for info in conditions:
            ...
            add_conditions = """INSERT INTO conditions(dvp_stage, subcondition, condition_type, subcondition_type) VALUES (NULL , '%s', '%s', '%s')""" % (
                subcondition, cond_type, subcondition_type)
            try:
                            ...
                            if count != 0:
                                try:
                                    ...
                                    try:
                                        ...
                                        select_sequence = """SELECT COUNT(*) FROM aligned_sequence WHERE sequence = '%s' and alignment_idalignment = '%s' and genes_idgenes = '%s';""" % (
                                            sequence, id_alignment, idgene)
                                        ...
                                        if count == 0:
                                            try:
                                                ...
                                                add_sequence = """INSERT INTO aligned_sequence(sequence, alignment_idalignment) VALUE('%s', '%s')""" % (
                                                    sequence, id_alignment)
        ...
        for gene in genes.keys():
            ...
            try:
                ...
                try:
                    ...
                    try:
                        select_gene = """SELECT COUNT(*) FROM genes WHERE genes_name is NULL and Ensembl_ID = '%s' and species_idspecies = '%s' and gene_family_idgene_family1 = '%s';""" % (
                            gene, id_species, id_family)
                        self._cursor.execute(select_gene)
                        count = self._cursor.fetchone()[0]
                    except mysql.connector.Error as err:
                        print(Fore.RED + str(err))
    ...
    def import_expressionslevel(self, family: str):
        ...
        for i in expr:
            ...
            exp_level = float(i[3])
            try:
                select_exprlvl = """SELECT COUNT(*) FROM expressionlevel WHERE length = %s AND eff_count = %s AND est_count = %s AND expression_level = %s;""" % (
                    lenght, eff_lenght, est_counts, exp_level)
                ...
                if count == 0:
                    try:
                        ...
                        add_expressionlevel = """INSERT INTO expressionlevel(length, eff_count, est_count, expression_level) VALUES('%s', '%s', '%s', '%s');""" % (
                            lenght, eff_lenght, est_counts, exp_level)
                            ...
                            try:
                                ...
                                try:
                                    add_gene_has_exp = """INSERT INTO gene_has_expression(genes_idgenes, expressionlevel_idexpressionlevel, expression_method_idexpression_method, conditions_idconditions, organs_idorgans) VALUES('%s', '%s', '%s', '%s', '%s')""" % (
                                        id_gene, id_exp_lvl, id_exp_mthd, id_condition, id_organ)
